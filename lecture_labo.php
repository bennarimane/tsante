...
if(isset($_SESSION['id_us']) AND !empty($_SESSION['id_us'])) {
   if(isset($_GET['id'])) {
      ...
      $msg = $bdd->prepare('SELECT * FROM bilan WHERE id = ? AND id_us = ?');
...
<br /><br />
   ...
   <div align="center">
     ...
     <h3> <b><?= $p_exp ?></b> vous a envoyé: </h3>  &nbsp;&nbsp; &nbsp;&nbsp; &nbsp;&nbsp; <?= $m['date_rps'] ?><br />
      <h3><b>Objet:</b> <?='Reponse' ?></h3>
      <br />
      <h3><b>Message:</b>  <?= nl2br($m['repnse']) ?></h3><br />
      <?php } ?>
       <a href="rsp_labo.php">Retour</a>  &nbsp;&nbsp; &nbsp;&nbsp; &nbsp;&nbsp;      <a href="supprimer1.php?id=<?= $m['id'] ?>" class="btn btn-danger btn-sm">Supprimer</a><br />
   </div>
</div>
      </div>
   </div>
...
<?php
      $lu = $bdd->prepare('UPDATE bilan SET see = 1 WHERE id = ?');
