...
$reqmail = $bdd->prepare("SELECT * FROM medecin WHERE id_med = ? ");
               ...
               if($mailexist == 0) {
                ...
                $ins = $bdd->prepare("INSERT INTO medecin(presentatoin,id_med) VALUES(?,?)");
                     $ins->execute(array($avec, $_SESSION['id']));
               
             
                 }else{  $ins= $bdd->prepare("UPDATE medecin SET presentation= ? WHERE id_med = ?");
                    $ins->execute(array($avec,$_SESSION['id']));} 
...
$result = mysqli_query($connexion," SELECT personne.id as id, personne.nom ,email,num_tlfn,adresse,jour,h_ov,h_f,presentation FROM personne,info_medecin,medecin where  personne.id =info_medecin.id_med AND medecin.id_med=personne.id AND  personne.id='{$_SESSION['id']}'"); 
...
?>
            ...
            <div class="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                            ...
                            </div>  <?php  echo $medecin["presentation"]; ?><form  class="user" method="POST" action="" enctype="multipart/form-data">
