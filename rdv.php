...
$result = mysqli_query($connexion," SELECT personne.nom as med,age,heure,journe,sexe,rendez.nom ,prenom FROM rendez,personne where id_us='{$_SESSION['id_us']} 'AND personne.id =rendez.id_med AND accept=1");
  ...
  $nbpharma=mysqli_num_rows($result);
   ...
   echo '	<table class="table table-striped table-hover user-list fixed-header">
		<thead>
			...
			<th><div>Sexe</div></th>	
			<th><div>Heure</div></th>
			<th><div>Date</div></th><th><div>Medecin</div></th>
			<th><div></div></th>
		</thead>
		<tbody>';
...
echo "<span><strong><em>".$pharma["sexe"]."</em></strong></span></div>\n";

echo "</td>\n";
           echo "  <td>\n";

echo "<span><strong><em>".$pharma["heure"]."</em></strong></span></div>\n";

echo "</td>\n";
           echo "  <td>\n";

echo "<span><strong><em>".$pharma["journe"]."</em></strong></span></div>\n";

echo "</td>\n";
          echo "  <td>\n";

echo "<span><strong><em>".$pharma["med"]."</em></strong></span></div>\n";
...
$resulta= mysqli_query($connexion," UPDATE `rendez` SET lu=1 WHERE id_us='{$_SESSION['id_us']}'");
?>
            </div>
		</div>
	</div>
